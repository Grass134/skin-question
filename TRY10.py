...
def save_results_batch():
    ...
    try:
        ...
        st.toast(f"✅ 批量保存{len(rows)}条数据到Google Sheets", icon="✅")
    except Exception as e:
            ...
            if st.session_state.ai_same_as_initial:
                ...
                if st.button("✅ 确认结果并进入下一题", key=f"btn_{idx}"):
                    ...
                    if initial_correct and not final_correct:
                        ...
                        decision_path = "同对（坚持）"  # 替换“固执”
                    else:
                        decision_path = "盲从（错改错）"  # 统一格式
                    ...
                    result = {
                        ...
                        "decision_path": decision_path,  # 新命名
                        "is_misled": is_misled,
...
def result_step():
    ...
    if len(st.session_state.user_results) > 0:
        ...
        display_df.columns = ["图片ID", "真实诊断", "你的初始诊断", "你的最终诊断", "AI建议", "决策路径"]
